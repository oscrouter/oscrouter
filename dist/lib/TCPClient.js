...
class TCPClient extends _SocketServer2.default {
  constructor(bind, port) {
    ...
    this.client.on("bundle", (oscBundle, timeTag, info) => {
      this.bundle(oscBundle, timeTag, {
        address: this.bind,
        port: this.port
      });
    });
    this.client.on("message", (message, timeTag, info) => {
      this.message(message, timeTag, {
        address: this.bind,
        port: this.port
      });
    });
