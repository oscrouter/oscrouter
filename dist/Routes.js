...
class Routes extends _events2.default {
  ...
  addRoute(uuid, settings) {
    ...
    this.getServer(incomingtype, incomingip, incomingport); //Theres no need to create a specific outgoing udp since its udp
  ...
  message(message, timeTag, remoteInfo, localInfo, type, sha) {
    ...
    _logger2.default.log('debug', `A ${socketType} OSC message just arrived from ${remoteInfo.address} to ${localInfo.port}`, {
    ...
    (0, _eachOf2.default)(routes, (route, uuid, next) => {
      ...
      if (route.outgoingtype === 'udp') {
        ...
        _logger2.default.log('debug', `Sending UDP OSC message to ${outgoingip}:${outgoingport}`, {
          address: o.reverse(params),
          args: message.args
        });
        ...
        _logger2.default.log('debug', `Sending TCP OSC message to ${stype}`, {
          address: o.reverse(params),
          args: message.args
        });
